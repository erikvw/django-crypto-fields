...
from django_crypto_fields.constants import LOCAL_MODE, AES
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            if settings.AES_ENCRYPTION_MODE != AES_CIPHER.MODE_CFB:
                raise CommandError('{} Got \'{}\''.format(
                    error_msg, settings.AES_ENCRYPTION_MODE))
    ...
    def update_crypts(self):
        ...
        crypts = django_apps.get_model(
            *app.model).objects.filter(algorithm=AES, cipher_mode=AES_CIPHER.MODE_CFB)
        updated = 0
        skipped = 0
        total = crypts.count()
        ...
        for index, obj in enumerate(crypts):
            ...
            if value:
                ...
                skipped += 1
            sys.stdout.write(
                '  ' + self.msg(total, index + 1, updated, skipped))
        sys.stdout.write('\n')
